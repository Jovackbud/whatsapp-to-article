...
class DocumentHandler:
    ...
    def extract_text_from_txt(file_content: bytes) -> str:
        """
        Extract text from TXT file, trying multiple encodings.
        ...
        """
        try:
            # Try encodings in order; latin-1 is a catch-all for any byte sequence
            ...
            for encoding in encodings:
                try:
                    ...
                    continue
            # Defensive safeguard — latin-1 decodes any byte, so this is technically unreachable
            raise RuntimeError("Unable to decode file with any supported encoding")
        except RuntimeError:
            raise
    ...
    def extract_text_from_pdf(file_content: bytes) -> str:
        ...
        try:
            ...
            if not text_content:
                logger.warning("pdfplumber extracted no text, falling back to pypdf.")
                return DocumentHandler._extract_text_pypdf(file_content)

            return '\n'.join(text_content)
        except Exception as e:
            logger.warning(f"pdfplumber failed, falling back to pypdf. Error: {e}")
            try:
                return DocumentHandler._extract_text_pypdf(file_content)
            except Exception as pypdf_e:
                logger.exception(f"Both pdfplumber and pypdf failed: {e} | {pypdf_e}")
                raise RuntimeError(f"Error reading PDF: Both extraction methods failed. {pypdf_e}")

    @staticmethod
    def _extract_text_pypdf(file_content: bytes) -> str:
        """
        ...
        Raises:
            RuntimeError: If there's an error reading the PDF with pypdf.
        """
        try:
            ...
            logger.exception(f"Error reading PDF with pypdf: {e}")
            raise RuntimeError(f"Error reading PDF with pypdf: {e}")
    ...
    def extract_text_from_file(uploaded_file: Any) -> str:
        ...
        try:
            ...
            if file_extension == 'txt':
                ...
                logger.error(f"Unsupported file type after validation: {file_extension}")
